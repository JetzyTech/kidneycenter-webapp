...
import { APIProvider, Map, Marker, AdvancedMarkerProps, useAdvancedMarkerRef, AdvancedMarker } from "@vis.gl/react-google-maps"
...
export default function Dashboard() {
	...
	return (
		...
		>
			<DashboardContext.Provider value={{ setHotelListings, hotelListingMutation }}>
				<div className="space-y-5">
					<div>
						<Typography.Text className="text-[32px] font-semibold">Explore Hotels</Typography.Text>
					</div>
					...
					{hotelListings?.docs?.length === 0 ||
						(hotelListings?.docs === undefined && (
							<div className="flex flex-col">
								<Typography.Text className="text-lg">No Hotels Found</Typography.Text>
								<Typography.Text className="text-sm">Please adjust your search criteria or try a different date range.</Typography.Text>
							</div>
						))}
						...
						<div className="flex items-start justify-between gap-x-10 w-full">
							<div className="space-y-5 w-2/5 h-[683px] overflow-y-scroll">
								{hotelListings?.docs?.map((entry) => (
									<div key={entry.id} className="cursor-pointer" onClick={() => onHotelSelect(entry.id)}>
							...
							{hotelListings?.docs?.length > 0 && (
								<div className="pr-10 w-3/5">
									<APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAP_API_KEY as string} libraries={["marker"]}>
										<Map key="1" defaultZoom={12} defaultCenter={{ lat: Number(lat), lng: Number(lng) }} style={{ width: "50vw", height: "683px" }}>
											{hotelListings?.docs &&
												hotelListings?.docs?.map((entry) => {
